...
moreEntriesButton.innerHTML = "Montrer " + entriesLimit + " de plus"
...
function highlightEntryFromUrl() {
	...
	if (id) {
		...
		if (entries.length === 1) {
			...
			document.title = entry.name + " sur l'Atlas"
			...
			if ((!entry.diff || entry.diff !== "delete")) {
				if (document.getElementById("objectEditNav")) {
					...
					document.getElementById("objectEditNav").title = "Modifier " + entry.name
				} else {
					...
					objectEditNav.innerText = "Modifier"
					objectEditNav.href = "./?mode=draw&id=" + id
					objectEditNav.title = "Modifier " + entry.name
