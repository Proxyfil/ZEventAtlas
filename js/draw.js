...
function initDraw() {
	...
	objectInfoForm.addEventListener('submit', function (e) {
		...
		if (e.submitter && e.submitter.value == "Prévisualiser") {
			preview()
		} else {
			exportJson()
		}
	...
	function reset() {
		...
		if (resetButton.textContent == "Confirmer l'annulation") {
			resetButton.textContent = "Annuler"
			resetButton.className = "btn btn-secondary"
			...
			resetButton.textContent = "Confirmer l'annulation"
	...
	function renderBackground() {
		...
		backgroundContext.clearRect(0, 0, window.canvasSize.y, window.canvasSize.x)
	...
	function render(path) {
		...
		context.clearRect(0, 0, window.canvasSize.y, window.canvasSize.x)
		...
		context.fillRect(0, 0, window.canvasSize.y, window.canvasSize.x)
	...
	function addWebsiteFields(link, index, array) {
		...
		inputField.title = "Site URL utilisant le protocol http:// ou https://"
		...
		if (array.length == index + 1) {
			...
			inputButton.title = "Ajouter un site"
			...
			inputButton.className = "btn btn-outline-secondary"
			inputButton.title = "Retirer le site"
			inputButton.innerHTML = '<i class="bi bi-trash-fill" aria-hidden="true"></i>'
	...
	if (params.has('id')) {
		...
		document.getElementById("offcanvasDrawLabel").textContent = "Nouvelle entrée"
		pathWithPeriods.push([defaultPeriod, []])
	...
	pathWithPeriods.forEach(([period, path], index) => {
		...
		startPeriodEl.addEventListener('input', () => {
			if (path.length >= 3) {
				...
				if ((periodCenter[1] > 700) && (startPeriodEl.valueAsNumber <= variationsConfig[variation].expansions[1])) {
					// Second expansion
					startPeriodEl.value = variationsConfig[variation].expansions[1];
				} else if ((periodCenter[0] > 700) && (startPeriodEl.valueAsNumber <= variationsConfig[variation].expansions[0])) {
					// First expansion
		...
		endPeriodEl.addEventListener('input', () => {
			if (path.length >= 3) {
				periodCenter = calculateCenter(path)
				if ((periodCenter[1] > 700) && (endPeriodEl.valueAsNumber <= variationsConfig[variation].expansions[1])) {
					// Second expansion
					endPeriodEl.value = variationsConfig[variation].expansions[1];
				} else if ((periodCenter[0] > 700) && (endPeriodEl.valueAsNumber <= variationsConfig[variation].expansions[0])) {
					// First expansion
		...
		function endPeriodUpdate(value) {
			...
			if (endPeriodEl.value == endPeriodEl.min) {
				...
				if (path.length >= 3) {
					periodCenter = calculateCenter(path)
					if (periodCenter && (periodCenter[1] > 700) && (endPeriodEl.valueAsNumber <= variationsConfig[variation].expansions[1])) {
						// Second expansion
						endPeriodLeftEl.disabled = true
						endPeriodRightEl.disabled = false
					} else if (periodCenter && (periodCenter[0] > 700) && (endPeriodEl.valueAsNumber <= variationsConfig[variation].expansions[0])) {
						// First expansion
						endPeriodLeftEl.disabled = true
						endPeriodRightEl.disabled = false
					} else {
		...
		periodCopyEl.addEventListener("click", event => {
			...
			if (event.target.textContent === " Copier") {
				event.target.className = "period-copy btn btn-primary btn-sm flex-fill"
				event.target.innerHTML = '<i class="bi bi-clipboard-x" aria-hidden="true"></i> Terminer'
				periodClipboard.index = index
				periodClipboard.path = [...pathWithPeriods[index][1]]
				updatePeriodGroups()
			} else if (event.target.textContent === " Terminer") {
				event.target.className = "period-copy btn btn-secondary btn-sm flex-fill"
				event.target.innerHTML = '<i class="bi bi-clipboard" aria-hidden="true"></i> Copier'
	...
	periodGroupElements.forEach((elements, index) => {
		...
		if (periodClipboard.index !== null) {
			if (index !== periodClipboard.index) {
				periodCopyEl.innerHTML = '<i class="bi bi-clipboard-plus" aria-hidden="true"></i> Coller'
				if (JSON.stringify(pathWithPeriods[index][1]) === JSON.stringify(periodClipboard.path)) {
					// If contents are identical prevent pasting
					periodCopyEl.innerHTML = '<i class="bi bi-clipboard-check" aria-hidden="true"></i> Coller'
				...
				periodCopyEl.innerHTML = '<i class="bi bi-clipboard-x" aria-hidden="true"></i> Terminer'
			...
			// Default state
			periodCopyEl.innerHTML = '<i class="bi bi-clipboard" aria-hidden="true"></i> Copier'
			periodCopyEl.disabled = false
			...
			if (startPeriodEl.value == startPeriodEl.min) {
				...
				if (periodCenter && (periodCenter[1] > 700) && (startPeriodEl.valueAsNumber <= variationsConfig[periodVariationEl.value].expansions[1])) {
					// Second expansion
					startPeriodLeftEl.disabled = true
					startPeriodRightEl.disabled = false
				} else if (periodCenter && (periodCenter[0] > 700) && (startPeriodEl.valueAsNumber <= variationsConfig[periodVariationEl.value].expansions[0])) {
					// First expansion
					startPeriodLeftEl.disabled = true
					startPeriodRightEl.disabled = false
				} else {
			...
			if (endPeriodEl.value == endPeriodEl.min) {
				...
				if (periodCenter && (periodCenter[1] > 700) && (endPeriodEl.valueAsNumber <= variationsConfig[periodVariationEl.value].expansions[1])) {
					// Second expansion
					endPeriodLeftEl.disabled = true
					endPeriodRightEl.disabled = false
				} else if (periodCenter && (periodCenter[0] > 700) && (endPeriodEl.valueAsNumber <= variationsConfig[periodVariationEl.value].expansions[0])) {
					// First expansion
					endPeriodLeftEl.disabled = true
					endPeriodRightEl.disabled = false
				} else {
...
function updateErrors() {
	if (path.length === 0) {
		periodsStatus.textContent = "Aucun segment possible sur cette période !"
	}
	...
	periodGroupElements.forEach((el, index) => {
		...
		if (conflicts[index] !== undefined) {
			periodStatusEl.textContent += `Conflit de période avec le segment${conflicts[index].length === 1 ? "" : "s"} ${conflicts[index].join(", ")}.\n`
		}
		if (insufficientPaths[index] !== undefined) {
			periodStatusEl.textContent += `Nombre de segments insuffisant. Actuellement ${insufficientPaths[index]}, a besoins d'au moins 3.\n`
	...
	if (errorCount > 0) {
		periodsStatus.textContent = `Problème détecté. Veuiller consulter l'erreur ci-dessous.`
